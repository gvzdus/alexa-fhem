...
function open_ssh(aconfig) {
  if (!ssh_client) {
    ...
    ssh_client = exec( aconfig.ssh,
      ['-R', '1234:' + aconfig['bind-ip']+ ':' + aconfig.port, '-oServerAliveInterval=90', 'fhem.garnix.de']);
 //     { stdio: 'pipe' });
...
Server.prototype.run = function() {
  ...
  if( alexaConfig.applicationId )
    for( const i = 0; i < alexaConfig.applicationId.length; ++i ) {
  ...
  if( alexaConfig.oauthClientID )
    for( const i = 0; i < alexaConfig.oauthClientID.length; ++i ) {
  ...
  for( var connection of this._config.connections ) {
    var fhem = new FHEM(log, connection);
