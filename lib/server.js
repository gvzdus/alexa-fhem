...
const log = Logger.withPrefix('server');
...
Server.asProxy = function(as_proxy) { log.warn('Functionality not implemented in this version') };
Server.useProxy = function(as_proxy) { log.warn('Functionality not implemented in this version') };

Server.prototype._loadConfig = function() {

  // Look for the configuration file
  const configPath = User.configPath();
  log.debug ("_loadConfig: using " + configPath );
  ...
  if (!fs.existsSync(configPath)) {
    ...
    //return config;
      log.error("_loadConfig: Couldn't find a config.json file at '"+configPath+"'. Look at config-sample.json for an example.");
      process.exit(1);
  }
  ...
  try {
    ...
    log.error("_loadConfig: There was a problem reading your config.json file.");
    log.error("_loadConfig: Please try pasting your config.json file here to validate it: http://jsonlint.com");
    log.error("");
    throw err;
  }

  if( typeof config.alexa.applicationId !== 'object' && config.alexa.hasOwnProperty('applicationId'))
    config.alexa.applicationId = [config.alexa.applicationId];

  if( typeof config.alexa.oauthClientID !== 'object' && config.alexa.hasOwnProperty('oauthClientID'))
    config.alexa.oauthClientID = [config.alexa.oauthClientID];

  var username = config.alexa.username;

  log.debug("_loadConfig: Config parsed completely.");
    ...
    request.on('end', function() {
      try {
        ...
        verifyToken.bind(this)(event, function(ret, error) {
          ...
          response.setHeader("X-ProcTime", "alexafhem:" + (Date.now()-start));
          response.end(JSON.stringify(ret)); });

      } catch (error) {
        log.error( 'On handleRequest: ' + error + ' from ' + request.connection.remoteAddress );
        const errRsp = createError(ERROR_UNSUPPORTED_OPERATION);
        log.info('>>>> '+ JSON.stringify(errRsp));
        response.end(JSON.stringify(errRsp));
...
Server.prototype.open_ssh = function() {
  if (!ssh_client) {
    ...
    ssh_client = exec( alexaConfig.ssh,
      ['-C', '-R', '1234:' + alexaConfig['bind-ip']+ ':' + alexaConfig.port, '-oServerAliveInterval=90', '-p', '58824', 'fhem-va.fhem.de']);
    //     { stdio: 'pipe' });
...
Server.prototype.run = function() {
  ...
  if( alexaConfig['nat-pmp'] )
    open_pmp(this._config.alexa['nat-pmp']);

  if( alexaConfig['nat-upnp'] )
    open_upnp();

  if ( alexaConfig.publicSkill ) {
    this.open_ssh();
  }
  ...
  this.roomOfIntent = {};
  if( alexaConfig.applicationId )
    for( let i = 0; i < alexaConfig.applicationId.length; ++i ) {
      const parts = alexaConfig.applicationId[i].split( ':', 2 );
      if( parts.length === 2 ) {
        this.roomOfIntent[parts[0]] = parts[1].toLowerCase();
        alexaConfig.applicationId[i] = parts[0];
      }
    }
  if( alexaConfig.oauthClientID )
    for( let i = 0; i < alexaConfig.oauthClientID.length; ++i ) {
      const parts = alexaConfig.oauthClientID[i].split( ':', 2 );
      if( parts.length === 2 ) {
        this.roomOfIntent[parts[0]] = parts[1].toLowerCase();
        alexaConfig.oauthClientID[i] = parts[0];
      }
    }
  ...
  const startupCmds = alexaConfig.publicSkill ? [
    { cmd: "jsonlist2 TYPE=alexa", callback: setAcceptedToken },
    // TODO: Remove me later...
    { cmd: "jsonlist2 FHEM.Alexa", callback: setAcceptedToken }
  ] : undefined;
  ...
  for( var connection of this._config.connections ) {
    ...
    fhem.on( 'ALEXA DEVICE', function(fhem, n) {
      if( fhem.alexa_device ) {
        ...
        if (! this._config.alexa.disableCustomSkill) {
...
var accepted_token;
// Tokens expected from usage of public skill. Read from FHEM (FHEM.Alexa, bearerToken reading).
...
const setAcceptedToken = function (result) {
  ...
  if (fhemalexa.totalResultsReturned<1) {
    if (published_tokens.length===0)
      log.error('No Alexa device defined - this will not work');
    return;
  }
  const readings = fhemalexa.Results[0].Readings;
  let token;
  if (! readings.hasOwnProperty(BEARERTOKEN_NAME)) {
    // TODO: Remove me in future, this is FHEM.Alexa code
    if (! readings.hasOwnProperty("bearerToken")) {
      ...
      token = readings['bearerToken'].Value;
  } else {
...
var verifyToken = function(event, callback) {
  ...
  else if( event.directive && event.directive.payload && event.directive.payload.grantee)
  ...
  let ptoken = token;
  if (ptoken && ptoken.lastIndexOf('-')>=0)
    ptoken = ptoken.substring(ptoken.lastIndexOf('-')+1);
  ...
  if( event.directive && event.directive.header && event.directive.header.namespace === NAMESPACE_Authorization && event.directive.header.name === "AcceptGrant" ) {
    ...
    // 1) If we validated it via Amazon OAuth (justme1968 classic approach)
    // 2) Validation is completely turned of (gvz beta1)
    // 3) This token is in the array of bearerToken(s) we read from FHEM (instances).
    // TODO: Shall we also check for remoteAddr=127.0.0.1 in UC 3?
  } else if( (token === accepted_token && Date.now() < expires) ||
    this._config.alexa.trustAny === true ||
    this._config.alexa.publicSkill && published_tokens.indexOf(ptoken)>=0) {
    handler.bind(this)( event, callback );
    ...
    require('https').get( url, function(result) {
      ...
      result.on('end', function() {
        try {
          ...
          if( parsedData.error ) {
            ...
            log.info('accepted new token ' + token);
      ...
      log.error('Got error on OAuth validaton: '+ e.message);
      callback( createError(ERROR_INVALID_ACCESS_TOKEN), ERROR_INVALID_ACCESS_TOKEN );
    });
...
var handleCustom = function(event, callback) {
    ...
    if( event.request.type === 'LaunchRequest' ) {
      ...
      log.info( "Received intent name: " + intent_name );
...
var handler = function(event, callback) {
  ...
  try {
    ...
    switch (requestedNamespace) {
      ...
      default:
        log.error("Unsupported namespace in request: " + requestedNamespace);
    ...
    log.error("Exception in handling request: ", error);
  }// try-catch
...
var handleDiscovery = function(event) {
  ...
  switch (requestedName) {
    case REQUEST_DISCOVER_APPLIANCES :
      ...
      for( var di in this.devices ) {
        var device = this.devices[di];
        ...
        //console.log(device);
        const d = { applianceId: device.uuid_base.replace( /[^\w_\-=#;:?@&]/g, '_' ),
    ...
    default:
      log.error("Unsupported operation in handleDiscovery: " + requestedName);
...
var handleSystem = function(event) {
  ...
  switch (requestedName) {
    ...
    default:
      log.error("Unsupported operation in handleSystem: " + requestedName);
      response = handleUnsupportedOperation();
...
var handleControl = function(event) {
  ...
  switch (requestedName) {
    ...
    default:
      log.error("Unsupported operation in handleControl: " + requestedName);
      response = handleUnsupportedOperation();
...
var handleQuery = function(event) {
  ...
  switch (requestedName) {
    ...
    default:
      log.error("Unsupported operation in handleQuery: " + requestedName);
      response = handleUnsupportedOperation();
...
var handleAlexa = function(event) {
  ...
  switch (requestedName) {
    ...
    default:
      log.error("Unsupported operation in handleAlexa: " + requestedName);
      response = handleUnsupportedOperation();
...
var handleAuthorization = function(event) {
  ...
  switch (requestedName) {
    ...
    default:
      log.error("Unsupported operation in handleAuthorization: " + requestedName);
      response = handleUnsupportedOperation();
...
var handleReportState = function(event) {
  ...
  var mapping;
  if( (mapping = device.mappings.On) !== undefined ) {
    const current = device.fhem.reading2homekit(mapping, device.fhem.cached(mapping.informId));
  ...
  if( (mapping = device.mappings.Brightness) !== undefined ) {
    const current = device.fhem.reading2homekit(mapping, device.fhem.cached(mapping.informId));
  ...
  if( (mapping = device.mappings.TargetPosition) !== undefined ) {
    let current = device.fhem.reading2homekit(mapping, device.fhem.cached(mapping.informId));
    if( current !== undefined )
      context.properties.push( {
  ...
  if( (mapping = device.mappings.TargetTemperature) !== undefined ) {
    const current = device.fhem.reading2homekit(mapping, device.fhem.cached(mapping.informId));
    if( current !== undefined )
      context.properties.push( {
  ...
  if( (mapping = device.mappings.TargetHeatingCoolingState) !== undefined ) {
      context.properties.push( {
          "namespace": NAMESPACE_ThermostatController,
          "name": "thermostatMode",
          "value": "HEAT",
  ...
  if( (mapping = device.mappings.CurrentTemperature) !== undefined ) {
    const current = device.fhem.reading2homekit(mapping, device.fhem.cached(mapping.informId));
    if( current !== undefined )
      context.properties.push( {
  ...
  var endpoint = { "endpointId": event.directive.endpoint.endpointId};
          ...
          d.capabilities.push( {
                                 ...
                                 "properties": {
                                   ...
                                   "configuration": {
                                     ...
                                     "supportedModes": [ "AUTO", "MANUAL", "ECO", "BOOST", "COMFORT" ]
                                   },
                                   "proactivelyReported": false,
                                   "retrievable": true
                                 }
...
//        }
    ...
    default:
      log.error("Unsupported operation in handleDiscovery3: " + requestedName);
      response = handleUnsupportedOperation();
...
var handleThermostatController = function(event) {
  ...
  switch (requestedName) {
    case 'AdjustTargetTemperature':
      mapping = device.mappings.TargetTemperature;
