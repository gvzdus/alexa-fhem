...
const log = require("./logger").getLogger('server.js');
...
Server.prototype.startServer = function() {
  ...
  this.server.on('error', (res) => {
    ...
    if (res.syscall === "listen") {
      // Errors on listen make our server pretty useless..
      // TODO: process.exit(?)
      //reject (res);
    }
  });
...
Server.prototype.run = function() {
  ...
  for( var connection of this._config.connections ) {
    var fhem = new FHEM(log, connection, startupCmds);
...
const setAcceptedToken = function (result) {
  log.debug('Callback from reading FHEM.Alexa: ' + result);
  const fhemalexa = JSON.parse(result);
  if (fhemalexa.totalResultsReturned<1) {
    log.error('No FHEM.Alexa device defined - this will not work');
    return;
  }
  const readings = fhemalexa.Results[0].Readings;
  if (! readings.hasOwnProperty('bearerToken')) {
    log.warn('No reading "bearerToken" found in FHEM.Alexa - incoming Cloud requests cannot be validated.');
    return;
  }
  log.info ("BearerToken '" + readings.bearerToken.Value + "' read from FHEM.");
  published_tokens.push(readings.bearerToken.Value);
};
