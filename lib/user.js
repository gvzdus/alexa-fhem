...
async function runAutoconfig() {
  ...
  const bearerTokenRandomBytes = crypto.randomBytes(8);
  ...
  if (fs.existsSync(pubkey_path)) {
    ...
    console.log("  https://fhem.garnix.de/privacy/");
    console.log("... and the press Enter to register your key.");
    let goon = readline.question("Okay to register your public SSH-Key at fhem.garnix.de? [Hit Enter for okay, 'n' else] ");
    if (goon !== 'n') {
      // Most likely initial SSH call: Avoid prompt for adding HostKey
      const prc = execSync(config.alexa.ssh, ['-o', 'StrictHostKeyChecking=no', 'fhem.garnix.de', 'register',
      ...
      if (searchAlexa.totalResultsReturned < 1) {
        ...
        await FHEM_execute(log, conn, "attr " + alexaDeviceName + " userattr skillRegistrationKey bearerToken");
      } else {
        const attrs = searchAlexa.Results[0].Attributes;
        alexaDeviceName = searchAlexa.Results[0].Name;
        if (!attrs.hasOwnProperty('userattr')) {
          await FHEM_execute(log, conn, "attr " + alexaDeviceName + " userattr skillRegistrationKey bearerToken");
        } else {
          let userattrs = attrs.userattr.split(" ");
          if (userattrs.indexOf('skillRegistrationKey')) userattrs.push('skillRegistrationKey');
          if (userattrs.indexOf('bearerToken')) userattrs.push('bearerToken');
          await FHEM_execute(log, conn, "attr " + alexaDeviceName + " userattr " + userattrs.join(' '));
        }
      }
      await FHEM_execute(log, conn, "attr " + alexaDeviceName + " skillRegistrationKey " + registrationKey);
      await FHEM_execute(log, conn, "attr " + alexaDeviceName + " bearerToken " +
        bearerTokenRandomBytes.toString('hex').toUpperCase());

      const searchFHEMAlexaString = await FHEM_execute(log, conn, "jsonlist2 FHEM.Alexa").catch(
        a => console.log("ERROR" + a));
      const searchFHEMAlexa = JSON.parse(searchFHEMAlexaString);
      if (searchFHEMAlexa.totalResultsReturned < 1) {
        await FHEM_execute(log, conn, "define FHEM.Alexa dummy");
        await FHEM_execute(log, conn, "attr FHEM.Alexa event-on-change-reading state");
        await FHEM_execute(log, conn, "attr FHEM.Alexa webCmd start:stop:restart");
        await FHEM_execute(log, conn, "define FHEM.Alexa.DOIF DOIF ([FHEM.Alexa] eq \"start\") " +
          "(set FHEM.Alexa on, {system (\"" + process.argv[1] + " > /dev/null 2>&1 &\")}) " +
          "DOELSEIF ([FHEM.Alexa] eq \"stop\") " +
          "(set FHEM.Alexa off, {system (\"" + process.argv[1] + " -k > /dev/null 2>&1 &\")}) " +
          "DOELSEIF ([FHEM.Alexa] eq \"restart\") " +
          "(set FHEM.Alexa on, {system (\"" + process.argv[1] + " -k && " + process.argv[1] + " > /dev/null 2>&1 &\")}) "
          )
      }
