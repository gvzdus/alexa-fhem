...
async function runAutoconfig() {
  ...
  if (fs.existsSync(pubkey_path)) {
    ...
    if (goon !== 'n') {
      ...
      const alexaDevice = s_json.Results[0];
      ...
      let tokenStoreCmd;  // who to store bearerToken & registrationKey

      if (alexaDevHasStart) {
        // 39_alexa.pm with start capability...

        if (alexaDevice.Readings.hasOwnProperty('bearerToken')) {
          bearerTokenRandomBytes = Buffer.from(alexaDevice.Readings.bearerToken.Value, "hex")
        } else {
          ...
          s_json = JSON.parse(s_str);
          if (s_json.totalResultsReturned > 0 && s_json.Results[0].Readings.hasOwnProperty('bearerToken')) {
            bearerTokenRandomBytes = Buffer.from(s_json.Results[0].Readings.bearerToken.Value, "hex");
            await FHEM_execute(log, conn, "delete FHEM.Alexa.autostart");
          }
        }
        await FHEM_execute(log, conn, "attr " + alexaDevice.Name + " alexaFHEM-cmd " + process.argv[1]);
        tokenStoreCmd = "setreading " + alexaDevice.Name;
        ...
        const readings = s_json.Results[0].Readings;
        ...
        await FHEM_execute(log, conn, "attr " + FHEMAlexa + " readingList skillRegistrationKey bearerToken");
      ...
      await FHEM_execute(log, conn, tokenStoreCmd + " skillRegistrationKey " + registrationKey);
      await FHEM_execute(log, conn, tokenStoreCmd + " bearerToken " +
        bearerTokenRandomBytes.toString('hex').toUpperCase());
