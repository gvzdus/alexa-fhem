...
User.configPath = function() {
  ...
  return path.join(User.storagePath(), "alexa-fhem.cfg");
};
...
var log = require("./logger").Logger.withPrefix('user');
...
User.autoConfig = function () {
  return new Promise(function (resolve, reject) {
    ...
    (async () => {
        const finalError = await runAutoconfig(true);
        if (finalError) {
...
async function runAutoconfig(interactive) {
  const spath = User.storagePath();

  // FIRST STEP: Check for config.json and build up one, if missing
  const readline = interactive ? require('readline-sync') : function () { return "" };
  ...
  if (!fs.existsSync(configPath)) {
    if (fs.existsSync("./.alexa/config.json")) {
      console.log("alexa-fhem.cfg not existing, creating from ~/.alexa/config.json");
      try {
        config = JSON.parse(fs.readFileSync("./.alexa/config.json"));
      } catch (e) {
        console.log("... which is broken JSON, so from the scratch anyways...");
      }
    } else if (fs.existsSync("./config-sample.json")) {
      console.log("alexa-fhem.cfg not existing, creating from ../config-sample.json");
      try {
      ...
      console.log("alexa-fhem.cfg not existing, creating from the scratch");
    }
    dirty = true;
  } else {
    try {
  ...
  if (!config.alexa.hasOwnProperty('disableCustomSkill')) {
    config.alexa.disableCustomSkill = true; dirty=true
  }

  if (config.alexa.ssh && config.alexa.ssh.trim()!==config.alexa.ssh) {
        ...
        switch (testReq.httpcode) {
          case 401:
            if (! conn.auth || ! conn.auth.user) {
              ...
              console.log('(Username & password will be stored unencrypted in the ~/alexa-fhem.cfg file - sorry)');
            } else {
    ...
    const execSync = require('child_process').spawnSync;
      ...
      if (alexaDevHasStart) {
        ...
        if (alexaDevice.Attributes['alexaFHEM-cmd'] !== process.argv[1]) {
          ...
          await FHEM_execute(log, conn,
            "delete " + FHEMAlexa + ".autostart");
        }

      } else {
        // DOIF + DummyDevice - Version:

        // Search for Device FHEM.Alexa (TYPE=dummy) and create if not existing, otherwise modify
        const FHEMAlexa = "FHEM.Alexa";
        let s_str = await FHEM_execute(log, conn, "jsonlist2 FHEM.Alexa").catch(
          a => console.log("ERROR" + a));
        let s_json = JSON.parse(s_str);
        if (s_json.totalResultsReturned < 1) {
          await FHEM_execute(log, conn, "define " + FHEMAlexa + " dummy");
          s_str = await FHEM_execute(log, conn, "jsonlist2 FHEM.Alexa").catch(
            a => console.log("ERROR" + a));
          s_json = JSON.parse(s_str);
        }
        const attributes = s_json.Results[0].Attributes;
        if (attributes['event-on-change-reading'] !== 'state')
          await FHEM_execute(log, conn, "attr " + FHEMAlexa + " event-on-change-reading state");
        if (attributes['webCmd'] !== 'Start:Stop:Reload')
        ...
        await FHEM_execute(log, conn, "define " + FHEMAlexa + ".DOIF DOIF ([" + FHEMAlexa + "] eq \"Start\") " +
          "(set " + FHEMAlexa + " on, {system (\"" + process.argv[1] + " -l > /tmp/alexa.stdout.log 2>&1 &\")}) " +
          "DOELSEIF ([" + FHEMAlexa + "] eq \"Stop\") " +
          "(set " + FHEMAlexa + " off, {system (\"" + process.argv[1] + " -k > /dev/null 2>&1 &\")}) " +
          "DOELSEIF ([" + FHEMAlexa + "] eq \"Reload\") " +
          "(set " + alexaDeviceName + " reload , set " + FHEMAlexa + " on )"
        );
        await FHEM_execute(log, conn, "attr " + FHEMAlexa + ".DOIF do always");
