...
function FHEM(log, config, startupCmd) {
  ...
  if( auth ) {
    ...
    if (config.ssl || use_ssl) {
      // Set rejectUnauthorized for any SSL connection, no matter if auth or not
...
FHEM_update(informId, orig, no_update) {
  ...
  //logger.info('  ' + date + ' caching: ' + informId + ': ' + orig );
...
FHEM_reading2homekit(mapping, orig)
{
  ...
  mapping.log.debug('    caching: ' + (mapping.name?'Custom '+mapping.name:mapping.characteristic_type) + (mapping.subtype?':'+mapping.subtype:'') + ': '
...
function FHEM_startLongpoll(connection, startupCmds) {
  ...
  connection.log.info( 'starting longpoll: ' + url );
...
//console.log( 'Rcvd: ' + (l.length>132 ? l.substring(0,132)+'...('+l.length+')':l) );
                   ...
                   if( l.substr(0,1) == '[' ) {
                     try {
                       ...
                       connection.log.info( '  longpoll JSON.parse: ' + err );
                       continue;
                     }
                   } else
                     d = l.split('<<', 3);
...
//console.log( 'value: ' + value );
             ...
             connection.log.info( 'longpoll ended, reconnect in: ' + timeout + 'msec' );
             ...
             connection.log.info( 'longpoll error: ' + err + ', retry in: ' + timeout + 'msec' );
             setTimeout( function(){FHEM_startLongpoll(connection)}, timeout );
        ...
        if( p.length == 2 )
          ...
          if( this.mappings[param] !== undefined ) {
            try {
              ...
              this.log.error("On homebridgemapping " + this.mappings[param] + ": " + err, err);
...
FHEM.prototype.checkAndSetGenericDeviceType = function() {
    this.log.debug('Checking devices and attributes...');
...
FHEMDevice(platform, s) {
  ...
  if( match = s.PossibleSets.match(/(^| )desired-temp(:[^\d]*([^\$ ]*))?/) ) {
    ...
    this.mappings.TargetHeatingCoolingState = { reading: 'mode',
                                                  ...
                                                  cmds: ['CUSTOM:desiredTemperature boost', 'HEAT:desiredTemperature comfort', 'ECO:desiredTemperature eco', 'AUTO:desiredTemperature auto'] };


  } else if( match = s.PossibleSets.match(/(^| )desired(:[^\d]*(1[^\$ ]*))?/) ) {
    //PID20
    if( !this.service_name ) this.service_name = 'thermostat';
  ...
  this.log.debug( s.Internals.NAME + ' has' );
  for( var characteristic_type in this.mappings ) {
    ...
    for( var mapping of mappings ) {
      if( characteristic_type == 'On' )
        this.log.debug( '  ' + characteristic_type + ' [' + (mapping.device ? mapping.device +'.':'') + mapping.reading + ';' + mapping.cmdOn +',' + mapping.cmdOff + ']' );
      else if( characteristic_type == 'Hue' || characteristic_type == 'Saturation' )
        this.log.debug( '  ' + characteristic_type + ' [' + (mapping.device ? mapping.device +'.':'') + mapping.reading + ';' + mapping.cmd + ';0-' + mapping.max +']' );
      else if( mapping.name ) {
        if( characteristic_type == CustomUUIDs.Volume )
          this.log.debug( '  Custom ' + mapping.name + ' [' + (mapping.device ? mapping.device +'.':'') + mapping.reading +  ';' + (mapping.nocache ? 'not cached' : 'cached' )  +']' );
        else
          this.log.debug( '  Custom ' + mapping.name + ' [' + (mapping.device ? mapping.device +'.':'') + mapping.reading + ']' );
      } else
        this.log.debug( '  ' + characteristic_type + ' [' + (mapping.device ? mapping.device +'.':'') + mapping.reading + ']' );
    }
  }
